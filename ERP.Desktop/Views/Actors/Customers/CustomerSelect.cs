...
{
    public partial class CustomerSelect : BaseForm
    {
        VTSaleEntities db = DBContext.UnitDbContext;
        Guid editingID;
        TypeAssistant assistant;
        readonly bool _isReturnCustomer;
        string _customerName;

        List<Country> countries;
        List<Person> people;
        public Guid ReturnedCustomerID { get; set; }
        public bool Selected { get; set; }
        CustomerServices _services;

        public CustomerSelect(bool isReturnCustomer, string customerName = null)
        {
            InitializeComponent();
            ...
            StyleDataGridViews(dgv_customer);
            _services = new CustomerServices(db);
        }
        public CustomerSelect() : this(false)
        {
        ...
        private void Customers_Load(object sender, EventArgs e)
        {
            FillDGV();
            FillCountries();
            if (_isReturnCustomer && !string.IsNullOrEmpty(_customerName))
            {
        ...
        void FillDGV()
        {
            ...
            dgv_customer.DataSource = people;
        }
        ...
        private void FillCountries()
        {
            ...
            if (pos != null)
            {
                try
                {
                    cmbCountries.SelectedValue = pos.Store.Branch.Area.City.CountryId;
                    cmbCities.SelectedValue = pos.Store.Branch.Area.CityId;
                    cmbAreas.SelectedValue = pos.Store.Branch.AreaId;
                }
                catch (Exception)
                {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                AlrtMsgs.EnterVaildData("اسم العميل");
                txtName.Focus();
                return;
            }
            ...
            if (db.PersonCategories.Where(x => !x.IsDeleted).Count() == 0)
            {
                ...
                {
                    AlrtMsgs.ChooseVaildData("خطأ اثناء اضافة فئة العملاء");
                    return;
                }
            }
            ...
            var person = new Person()
            {
                ...
                Mob2 = txtMob2.Text,
                Name = txtName.Text,
                PersonTypeId = (int)(rbCustomer.Checked ? Lookups.PersonTypeCl.Customer : Lookups.PersonTypeCl.SupplierAndCustomer)
            };
            ...
            if (_isReturnCustomer)
            {
                ReturnedCustomerID = result.Object.Id;
                Selected = true;
                Close();
            }
            else
        ...
        private void dgvCustomers_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == senderGrid.Columns[nameof(edit)].Index && e.RowIndex >= 0)
            {
                ...
                if (person != null)
                {
                    ...
                    (person.PersonTypeId == (int)Lookups.PersonTypeCl.Customer ? rbCustomer : rbCustAndSupplier).Checked = true;
        ...
        private void Search()
        {
            ...
            {
                dgv_customer.DataSource = people.Where(x => x.Name.Contains(txt_search.Text) || x.Mob1.Contains(txt_search.Text) || x.Mob2?.Contains(txt_search.Text) == true).ToList();
            }
            else
            {
                dgv_customer.DataSource = people;
        ...
        #region Select customer from DataGridView
        private void dgv_customer_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (_isReturnCustomer && e.RowIndex >= 0)
            {
                ReturnedCustomerID = Guid.Parse(dgv_customer.Rows[e.RowIndex].Cells[nameof(Person_ID)].Value + "");
                Selected = true;
                this.Close();
            }
        }
        ...
        {
            if (e.KeyData == Keys.Enter && _isReturnCustomer && dgv_customer.CurrentRow != null)
            {
                ReturnedCustomerID = Guid.Parse(dgv_customer.CurrentRow.Cells[nameof(Person_ID)].Value + "");
                Selected = true;
                this.Close();
            }
        }

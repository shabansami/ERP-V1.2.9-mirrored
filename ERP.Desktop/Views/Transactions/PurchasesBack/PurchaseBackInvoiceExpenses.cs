...
{
    public partial class PurchaseBackInvoiceExpenses : BaseForm
    {
        Guid editingID;
        Guid invoiceID;
        public bool IsChanged = false;
        VTSaleEntities db = DBContext.UnitDbContext;
        public PurchaseBackInvoiceExpenses(Guid invoiceId)
        {
            InitializeComponent();
        ...
        void FillDGV()
        {
            var list = db.PurchaseBackInvoicesExpenses.Where(x => !x.IsDeleted && x.PurchaseBackInvoiceId == invoiceID).Select(x => new ExpensIncomeVM
            {
                ID = x.Id,
                Amount = x.Amount,
                TypeID = x.ExpenseTypeAccountsTreeID ?? Guid.Empty,
                TypeName = x.ExpenseTypeAccountsTree != null ? x.ExpenseTypeAccountsTree.AccountName : ""
            }).ToList();
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            ...
            db.PurchaseBackInvoicesExpenses.Add(new PurchaseBackInvoicesExpens()
            {
                Amount = amount,
                ExpenseTypeAccountsTreeID = typeID,
                PurchaseBackInvoiceId = invoiceID
            });
        ...
        private void dgExpensesInvoices_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (e.ColumnIndex == senderGrid.Columns[nameof(delete)].Index)
            {
                ...
                {
                    var income = db.PurchaseBackInvoicesExpenses.FirstOrDefault(x => x.Id == editingID);
                    if (income != null)
                    {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            if (income != null)
            {
                ...
                income.ExpenseTypeAccountsTreeID = typeID;
                db.SaveChanges(UserServices.UserInfo.UserId);
                IsChanged = true;
                FillDGV();
                ClearForm();

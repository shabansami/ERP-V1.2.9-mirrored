...
{
    public partial class PurchaseBackInvoicesCashier : BaseForm
    {
        ...
        PurchaseBackInvoice currentInvoice;
        Item currentItem;
        List<ItemDetailsDT> CurrntInvoiceItems;


        readonly VTSaleEntities db;
        readonly PurchaseBackServices _services;
        ...
        Guid? StoreID = null;
        bool loading = false;
        public PurchaseBackInvoicesCashier() : base()
        {
            InitializeComponent();
            ...
            _services = new PurchaseBackServices(db);
        }
        ...
        private void CashierSales_Load(object sender, EventArgs e)
        {
            if (LoadMainInfo() && LoadShiftInfo())
            {
                ...
                {
                    var form = new AllPurchaseBackInvoices(currentShift.Id, _services);
                    form.ShowDialog();
        ...
        private bool CreateNewInvoice()
        {
            ...
            var invoice = new PurchaseBackInvoice()
            {
        ...
        private void LoadInvoice(PurchaseBackInvoice invoice)
        {
            loading = true;
            ClearForm();
            if (invoice != null)
            {
                ...
                txt_itemcount.Text = invoice.PurchaseBackInvoicesDetails.Count(x => !x.IsDeleted) + "";
        ...
        private void txtInvoicePaied_TextChanged(object sender, EventArgs e)
        {
            ...
            if (rest == 0)
            {
                txtInvoiceRemained.BackColor = SystemColors.Control;
            }
            else if (rest > 0)
            {
        ...
        private void btnSaveInvoice_Click(object sender, EventArgs e)
        {
            ...
            if (ckPrinting.Checked)
                PrintReport("فاتورة مرتجع توريدات");
        ...
        private void AddItem()
        {
            ...
            var itemDetails = new PurchaseBackInvoicesDetail()
            {
                Amount = quantity * price,
                Quantity = quantity,
                Price = price,
                ItemId = currentItem.Id,
                PurchaseBackInvoiceId = currentInvoice.Id,
        ...
        public void PrintReport(string title = " فاتورة مرتجع توريدات")
        {
            if (currentInvoice != null)
            {
        ...
        private void btnExpenes_Click(object sender, EventArgs e)
        {
            if (currentInvoice != null)
            {
                var form = new PurchaseBackInvoiceExpenses(currentInvoice.Id);
                form.ShowDialog();
                if (form.IsChanged)
                {
                    SaveInvoiceCalculationChanges();

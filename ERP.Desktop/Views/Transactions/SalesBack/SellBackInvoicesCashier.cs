...
{
    public partial class SellBackInvoicesCashier : BaseForm
    {
        ...
        SellBackInvoice currentInvoice;
        Item currentItem;
        List<ItemDetailsDT> CurrntInvoiceItems;


        readonly VTSaleEntities db;
        readonly SaleBackServices _services;
        ...
        Guid? StoreID=null;
        bool loading = false;
        public SellBackInvoicesCashier() : base()
        {
            InitializeComponent();
            ...
            _services = new SaleBackServices(db);
        }
        ...
        private void CashierSales_Load(object sender, EventArgs e)
        {
            if (LoadMainInfo() && LoadShiftInfo())
            {
                ...
                {
                    var form = new AllSellBackInvoices(currentShift.Id, _services);
                    form.ShowDialog();
        ...
        private void LoadInvoice(SellBackInvoice invoice)
        {
            loading = true;
            ClearForm();
            if (invoice != null)
            {
                ...
                //cmb_invotype.SelectedValue = ; //TODO:Invoice Type

                txt_itemcount.Text = invoice.SellBackInvoicesDetails.Count(x => !x.IsDeleted) + "";
        ...
        private void btnSaveInvoice_Click(object sender, EventArgs e)
        {
            ...
            if (ckPrinting.Checked)
                PrintReport("فاتورة مرتجع مبيعات");
        ...
        private void AddItem()
        {
            ...
            var itemDetails = new SellBackInvoicesDetail()
            {
                Amount = quantity * price,
                Quantity = quantity,
                Price = price,
                ItemId = currentItem.Id,
                SellBackInvoiceId = currentInvoice.Id,
        ...
        private void CashierSales_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                //CashierReturn @return = new CashierReturn();
                //@return.ShowDialog();
            }
        ...
        public void PrintReport(string title = " فاتورة مرتجع مبيعات")
        {
            if (currentInvoice != null)
            {
        ...
        private void btnExpenes_Click(object sender, EventArgs e)
        {
            if (currentInvoice != null)
            {
                var form = new SellBackInvoiceIncomes(currentInvoice.Id);
                form.ShowDialog();
                if (form.IsChanged)
                {
                    SaveInvoiceCalculationChanges();
        ...
        private void btn_sellInvoice_Click(object sender, EventArgs e)
        {
            ShowSellInvoices showSell = new ShowSellInvoices();
            showSell.ShowDialog();
            if (showSell.selected)
            {
                if (showSell.sellDetailIds.Count()>0)
                {
                    AddItemFromSellInvoice(showSell.sellDetailIds);
                }
            }
        }

        #region مرتجع من فاتورة بيع 
        private void AddItemFromSellInvoice(List<Guid> items)
        {
            //check validation of invoice
            if (currentInvoice != null)
            {
            ...
            {
                var invoiceDetails = db.SellInvoicesDetails.Where(x => !x.IsDeleted && x.Id == item).FirstOrDefault();
                if(invoiceDetails != null)
                {
                    ...
                    var itemDetails = new SellBackInvoicesDetail()
                    {
                        ...
                        SellBackInvoiceId = currentInvoice.Id,
                        ItemDiscount = invoiceDetails.ItemDiscount,
                        StoreId = StoreID
                    };

...
var Asset_Module = function () {
    var initDT = function () {
        ...
        table.DataTable({
            ...
            buttons: [
                ...
                {
                    "extend": "excelHtml5",
                    "filename": "الأصــول الثابتة",
                    "title": "الأصــول الثابتة",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
    ...
    function SubmitForm(btn, isIntialBalanceJs) {
        try {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (isIntialBalanceJs === "1")
                            setTimeout(function () { window.location = "/FixedAssetIntialBalances/CreateEdit" }, 3000);
                        else
                            setTimeout(function () { window.location = "/FixedAssets/CreateEdit" }, 3000);
    ...
    function onIsCarStoreChange() {
        if ($("#IsCarStore").is(":checked")) {
            $("#IsNotCarStore").prop("checked", false);
        };
    };
    function onIsNotCarStoreChange() {
        if ($("#IsNotCarStore").is(":checked")) {
            $("#IsCarStore").prop("checked", false);
        };
    };
    function onIsDestructionChange() {
        if ($("#IsDestruction").is(":checked")) {
            $("#divDestruction").show();
        } else
            $("#divDestruction").hide();
    };
    function destructionClc() {
        if ($("#IsDestruction").is(":checked")) {
            var scrapValue = $("#ScrapValue").val();
            ...
            if (isNaN(amount))
                amount = 0;
            var val =parseFloat((amount - scrapValue) / usefulLife).toFixed(2);
            $("#DestructionAmount").val(val);
        }
    }
    return {
        ...
        onIsCarStoreChange: onIsCarStoreChange,
        onIsNotCarStoreChange: onIsNotCarStoreChange,
        onIsNotCarStoreChange: onIsNotCarStoreChange,
        onIsDestructionChange: onIsDestructionChange,
        destructionClc: destructionClc,
    };

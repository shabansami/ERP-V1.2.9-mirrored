"use strict";
var BalanceSupplier_Module = function () {
    var initDT = function () {
        var table = $('#kt_datatable').DataTable({
            // begin first table
                ...
                buttons: [
                    {
                        ...
                        title: function () {
                            ...
                            var div = "<div style='color:black;' >ارصدة موردين</div>"
                            return div;
                        },
                ...
                ajax: {
                    url: '/RptAccountStatements/SearchBalancePersons',
                    type: 'GET',
                    data(d) {
                        d.DrpPersonId = $("#DrpPersonId").val();
            ...
            columns: [
                {
                    ...
                    { data: 'AccountsTreeName', title: ' الحساب' },
                    { data: 'LastBalanceDebit', title: ' رصيد حركة الحساب قبل الفترة مدين' },
                    { data: 'LastBalanceCredit', title: ' رصيد حركة الحساب قبل الفترة دائن' },
                    { data: 'TotalDebit', title: ' اجمالى الفترة المحددة مدين' },
                    { data: 'TotalCredit', title: ' اجمالى الفترة المحددة دائن' },
                    { data: 'TotalBalanceDebit', title: ' رصيد حركة الحساب خلال الفترة مدين' },
                    { data: 'TotalBalanceCredit', title: ' رصيد حركة الحساب خلال الفترة دائن' },
                    { data: 'SafyBalanceDebit', title: ' الرصيد النهائى للحساب حتى نهاية الفترة مدين' },
                    { data: 'SafyBalanceCredit', title: ' الرصيد النهائى للحساب حتى نهاية الفترة دائن' },
            ...
            drawCallback: function () {
                var html = ' <tr><th colspan ="12" style= "text-align:center" ><div class="row alert-success"><label>اجمالى الرصيد مدين : ';
                var api = this.api();
                var totalDebit = api.column(11).data().sum();
                var totalCredit = api.column(12).data().sum();
                $(api.table().footer()).html(html + totalDebit + '</label></div></th> <th colspan ="12" style= "text-align:center" ><div class="row alert-success"><label>اجمالى الرصيد دائن :' + totalCredit + '</label ></div ></th > </tr>');
            },
    ...
    function format(d) {
        var tr = '';
        $.each(d.GeneralDalies, function (index, data) {
            tr += ' <tr>' +

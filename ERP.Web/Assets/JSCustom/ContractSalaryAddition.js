...
var ContractSalaryAddition_Module = function () {
    var initDT = function () {
        ...
        table.DataTable({
            ...
            buttons: [
                ...
                {
                    extend: "excelHtml5",
                    filename: "إضافات الموظفين",
                    title: "إضافات الموظفين",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
            ...
            ajax: {
                url: '/ContractSalaryAdditions/GetAll',
                type: 'GET',

            },
            columns: [
                { data: 'Num', responsivePriority: 0 },
                { data: 'SalaryAddition', title: 'الإضافة' },
                { data: 'Employee', title: 'الموظف' },
                { data: 'Amount', title: 'مبلغ الاضافة' },
                { data: 'AdditionDate', title: 'تاريخ العملية' },
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
						...
						';
                            ...
                            return '\
							<div class="btn-group">\
                            <span class="label label-lg font-weight-bold label-light-success label-inline">تم صرفها</span></div>\
						';
                        }
                    },
                }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/ContractSalaryAdditions/Index" }, 3000);
                        } else
                            setTimeout(function () { window.location = "/ContractSalaryAdditions/CreateEdit" }, 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/ContractSalaryAdditions/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {

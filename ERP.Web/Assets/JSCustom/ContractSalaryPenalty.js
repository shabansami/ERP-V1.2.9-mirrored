...
var ContractSalaryPenalty_Module = function () {
    var initDT = function () {
        ...
        table.DataTable({
            ...
            ajax: {
                url: '/ContractSalaryPenalties/GetAll',
                type: 'GET',

            },
            columns: [
                { data: 'Num', responsivePriority: 0 },
                { data: 'SalaryPenalty', title: 'الخصم' },
                { data: 'Employee', title: 'الموظف' },
                { data: 'Amount', title: 'مبلغ الخصم' },
                { data: 'PenaltyDate', title: 'تاريخ العملية' },
                { data: 'MonthYear', title: 'شهر/اسبوع/يوم' },
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (!row.IsPayed) {
                            return '\
							<div class="btn-group">\
							<a href="/ContractSalaryPenalties/Edit/'+ row.Id + '" class="btn btn-sm btn-clean btn-icon" title="تعديل">\
								<i class="fa fa-edit"></i>\
							</a>\
							<a href="javascript:;" onclick=ContractSalaryPenalty_Module.deleteRow(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="حذف">\
						...
						';
                            ...
                            return '\
							<div class="btn-group">\
                            <span class="label label-lg font-weight-bold label-light-success label-inline">تم خصمها</span></div>\
						';
                        }
                    },
                }
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/ContractSalaryPenalties/Index" }, 3000);
                        } else
                            setTimeout(function () { window.location = "/ContractSalaryPenalties/CreateEdit" }, 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/ContractSalaryPenalties/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
    ...
    function getEmployeesDepartmentChange() {
        $.get("/SharedDataSources/GetEmployeeByDepartment", { id: $("#DepartmentId").val() }, function (data) {
            $("#EmployeeId").empty();
    ...
    function getContractSchedulingEmpChange() {
        $.get("/SharedDataSources/GetContractSchedulingEmployee", { id: $("#EmployeeId").val() }, function (data) {
            ...
            $("#salaryTypeId").val(data.salaryTypeId);
            $("#salaryTypeName").text('نوع العقد : ' + data.salaryTypeName);
        });

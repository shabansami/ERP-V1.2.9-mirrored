...
var ContractSupplier_Module = function () {
    var initDT = function () {
        ...
        columns: [
                ...
                },
            {
                ...
                render: function (data, type, row, meta) {
                                return `
							<div class="btn-group">
                                <a href="/ContractSuppliers/Edit/`+ row.Id + `" class="btn btn-sm btn-clean btn-icon" title="تعديل">\
								<i class="fa fa-edit"></i>\
							</a>
							<a href="javascript:;" onclick=ContractSupplier_Module.deleteRow('`+ row.Id + `') class="btn btn-sm btn-clean btn-icUrln" title="حذف">\
...
//"order": [[0, "desc"]] 
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/ContractSuppliers/Index" }, 3000);
                        }else
                            setTimeout(function () { window.location = "/ContractSuppliers/CreateEdit" } , 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/ContractSuppliers/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {

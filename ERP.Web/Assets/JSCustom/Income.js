...
var Income_Module = function () {
    //#region Income
    var initDTIncome = function () {
        var table = $('#kt_datatableIncome');
        ...
        table.DataTable({
            ...
            ajax: {
                ...
                type: 'GET',
                data(d) {
                    d.incomeTypeId = $("#IncomeTypeId").val();
            ...
            columns: [
                ...
                { data: 'IncomeTypeName', title: 'نوع الإيراد' },
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (!row.IsApproval) {
                            return '\
							<div class="btn-group">\
							<a href="/Incomes/Edit/'+ row.Id + '" class="btn btn-sm btn-clean btn-icon"  title="تعديل">\
								<i class="la la-edit"></i>\
							</a>\
                                <a href="javascript:;" onclick=Income_Module.ApprovalIncome(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="اعتماد">\
						...
						';
                            ...
                            <span class="label label-lg font-weight-bold label-light-success label-inline">تم اعتمادها</span><a href="javascript:;" onclick=Income_Module.UnApprovalIncome(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="فك الاعتماد">\
								<i class="la la-check-square-o"></i>\<a href="/GeneralDailies/Index/?tranId='+ row.Id + '&tranTypeId=21" class="btn btn-sm btn-clean btn-icon" title="عرض القيود">\
        ...
        $('#export_print').on('click', function (e) {
            e.preventDefault();
            $('#kt_datatableIncome').DataTable().button('.buttons-print').trigger();
        });

        $('#export_copy').on('click', function (e) {
            e.preventDefault();
            $('#kt_datatableIncome').DataTable().button('.buttons-copy').trigger();
        });

        $('#export_excel').on('click', function (e) {
            e.preventDefault();
            $('#kt_datatableIncome').DataTable().button('.buttons-excel').trigger();
        });
    };
    function SubmitFormIncome(btn) {
        try {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/Incomes/Index" }, 3000);
                        } else
                            setTimeout(function () { window.location = "/Incomes/CreateEdit" }, 3000);
                        //$('#kt_datatableIncomeLast').DataTable().ajax.reload();
                    } else {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.isValid) {
                            ...
                            $('#kt_datatableIncome').DataTable().ajax.reload();
                        } else {
    ...
    return {
        ...
        initDTIncome: function () {
            initDTIncome();
        },
        SubmitFormIncome: SubmitFormIncome,
        deleteRowIncome: deleteRowIncome,
        ApprovalIncome: ApprovalIncome,
        UnApprovalIncome: UnApprovalIncome,

...
var SafeIntialBalance_Module = function () {
    var initDT = function () {
        ...
        table.DataTable({
            ...
            buttons: [
                ...
                {
                    extend: "excelHtml5",
                    filename: "رصيد أول المده للخزن",
                    title: "رصيد أول المده للخزن",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
            ...
            ajax: {
                url: '/SafeIntialBalances/GetAll',
                type: 'GET',

                    },
        columns: [
            { data: 'Num',responsivePriority:0 },
            { data: 'SafeName', title: 'اسم الخزنة' },
                ...
                },
            {
                ...
                render: function (data, type, row, meta) {
                                return '\
							<div class="btn-group">\
							<a href="javascript:;" onclick=SafeIntialBalance_Module.deleteRow(\''+ row.TransactionId + '\') class="btn btn-sm btn-clean btn-icUrln" title="حذف">\
...
//"order": [[0, "desc"]] 
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/SafeIntialBalances/Index" }, 3000);
                        }else
                            setTimeout(function () { window.location = "/SafeIntialBalances/CreateEdit" } , 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/SafeIntialBalances/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {

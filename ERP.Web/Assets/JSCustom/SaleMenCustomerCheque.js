...
var SaleMenCustomerCheque_Module = function () {
    var initDT = function () {
        ...
        table.DataTable({
            ...
            ajax: {
                url: '/SaleMenCustomerCheques/GetAll',
                type: 'GET',
            ...
            columns: [
                ...
                { data: 'CheckDate', title: 'تاريخ العملية' },
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (!row.IsApproval) {
                            return '\
							<div class="btn-group">\
							<a href="/SaleMenCustomerCheques/Edit/'+ row.Id + '" class="btn btn-sm btn-clean btn-icon"  title="تعديل">\
								<i class="fa fa-edit"></i>\
							</a>\
                                <a href="javascript:;" onclick=SaleMenCustomerCheque_Module.deleteRow(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="حذف">\
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/SaleMenCustomerCheques/Index" }, 3000);
                        } else
                            setTimeout(function () { window.location = "/SaleMenCustomerCheques/CreateEdit" }, 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/SaleMenCustomerCheques/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {

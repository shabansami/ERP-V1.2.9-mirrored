...
var SellBackInvoiceAccounting_Module = function () {

    //#region ======== Save sell invoice ==================
    var initDT = function () {
        ...
        table.DataTable({
            ...
            buttons: [
                {
                    extend: 'print',
                    title: function () {
                        return 'الاعتماد المحاسبى لفواتير مرتجع البيع';
                    },
                ...
                {
                    extend: "excelHtml5",
                    filename: "الاعتماد المحاسبى لفواتير مرتجع البيع",
                    title: "الاعتماد المحاسبى لفواتير مرتجع البيع",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
            ...
            ajax: {
                url: '/SellBackInvoiceAccounting/GetAll',
                type: 'GET',
                ...
                render: function (data, type, row, meta) {
                    if (row.IsApprovalAccountant) {
                       ...
                       return '\
						...
						<a href="javascript:;" onclick=SellBackInvoiceAccounting_Module.ApprovalInvoice(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="اعتماد الفاتورة محاسبيا">\
...
//"order": [[0, "desc"]] 
    ...
    function ApprovalInvoice(invoGuid) { // اعتماد فاتورة محاسبيا
        Swal.fire({
            ...
            if (result.value) {
                var url = '/SellBackInvoiceAccounting/ApprovalInvoice';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {

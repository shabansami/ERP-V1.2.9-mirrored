                ...
                },
            {
                ...
                render: function (data, type, row, meta) {
                    var ele = '\
							...
							</a>\<a href="/SellInvoices/CopyInvoice/?invoGuid='+ row.Id + '" class="btn btn-sm btn-clean btn-icon" title="نسخ">\
                    ...
                    if (row.IsFinalApproval) {
                        ...
                        ele += '<a href="javascript:;" onclick=SellInvoice_Module.ApprovalFinalInvoice(\'' + row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="اعتماد الفاتورة بشكل نهائى">\
                       	<i class="fa fa-check"></i>\
                        	</a><a href="/SellInvoices/Edit/?invoGuid=' + row.Id + '" class="btn btn-sm btn-clean btn-icon" title="تعديل">\
...
//"order": [[0, "desc"]] 
    ...
    function ApprovalFinalInvoice(invoGuid) { // اعتماد فاتورة بشكل نهائى
        Swal.fire({
            ...
            if (result.value) {
                var url = '/SellInvoices/ApprovalFinal';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {

...
var SellInvoicePayment_Module = function () {
    ...
    var initDT = function () {
        ...
        table.DataTable({
            ...
            buttons: [
                {
                    ...
                    title: function () {
                        ...
                        var div = "<div style='color:black;' >دفعات فواتير البيع</div>"
                        return div;
                    },
                ...
                {
                    "extend": "excelHtml5",
                    "filename": "دفعات فواتير البيع",
                    "title": "دفعات فواتير البيع",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
            ...
            columns: [
                ...
                { data: 'InvoiceNumber', title: 'رقم فاتورة البيع' },
                { data: 'CustomerName', title: 'العميل' },
                { data: 'InvoiceDate', title: 'تاريخ فاتورة البيع' },
                { data: 'OperationDate', title: 'تاريخ عملية السداد' },
                { data: 'Amount', title: 'المبلغ المسدد' },
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        return '\
							<div class="btn-group">\
							<a href="javascript:;" onclick=SellInvoicePayment_Module.deleteRow(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="حذف">\
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            ...
                            setTimeout(function () { window.location = "/SellInvoicePayments/CreateEdit" }, 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/SellInvoicePayments/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
    ...
    function onCustomerChange() {
        $.get("/SharedDataSources/GetSellInvoiceIds", { customerId: $("#CustomerId").val() }, function (data) {
            $("#SellInvoiceId").empty();
            $("#SellInvoiceId").append("<option value=>اختر عنصر من القائمة</option>");
            $.each(data, function (index, row) {
                $("#SellInvoiceId").append("<option value='" + row.Id + "'>" + row.Name + "</option>");
            });

        })
    };
    function onSellInvoiceChange() {
        $.get("/SharedDataSources/GetInvoiceAmount", { sellInvoiceId: $("#SellInvoiceId").val() }, function (data) {
            $("#InvoiceAmount").val(data.invoiceAmount);
            $("#TotalInvoiceAmount").val(data.totalInvoiceAmount);
            $("#RemindAmount").val(data.remindAmount);
        })
    };
    function onAmountChange() {
        ...
        if (isNaN(amount))
            amount = 0;
        var remindAmount = Number.parseFloat($("#RemindAmount").val());
        if (isNaN(remindAmount))
            remindAmount = 0;
        if (amount>remindAmount) {
            toastr.error('المبلغ المسدد اكبر من المتبقى', '');
    ...
    return {
        ...
        onSellInvoiceChange: onSellInvoiceChange ,

...
var StorePermissionsLeave_Module = function () {
    var initDT = function () {
        ...
        table.DataTable({
            ...
            ajax: {
                url: '/StorePermissionsLeave/GetAll',
                type: 'GET',
                    ...
                    render: function (data, type, row, meta) {
                        if (row.IsApproval) {
                            ...
                            <span class="label label-lg font-weight-bold label-light-success label-inline">تم اعتمادها</span><a href="javascript:;" onclick=StorePermissionsLeave_Module.Unapproval(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln d-none" title="فك الاعتماد">\
								<i class="la la-check-square-o"></i>\<a href="javascript:;" onclick=StorePermissionsLeave_Module.deleteRow(\''+ row.Id + '\') class="btn btn-sm btn-clean btn-icUrln" title="حذف">\
    ...
    function SubmitForm(btn) {
        try {
            ...
            $.ajax({
                type: 'POST',
                url: '/StorePermissionsLeave/CreateEdit',
                data: formData,
                contentType: false,
                processData: false,
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/StorePermissionsLeave/Index" }, 3000);
                        } else
                            setTimeout(function () { window.location = "/StorePermissionsLeave/CreateEdit" }, 3000);
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/StorePermissionsLeave/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
    ...
    function approval(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/StorePermissionsLeave/Approval';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
    ...
    function Unapproval(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/StorePermissionsLeave/UnApproval';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
    ...
    var initDTItemDetails = function () {
        ...
        table.DataTable({
            ...
            ajax: {
                url: '/StorePermissionsLeave/GetDSItemDetails',
                type: 'GET',
            ...
            columnDefs: [
                ...
                //},
                {
                    ...
                    render: function (data, type, row, meta) {
                        return '\
							<div class="btn-group">\
							<a href="javascript:;" onclick=StorePermissionsLeave_Module.deleteRowItemDetails('+ row.Id + ')  class="btn btn-sm btn-clean btn-icUrln deleteIcon" title="حذف">\
								<i class="la la-trash"></i>\
							</a></div>\
						';
                    },
    ...
    function addItemDetails() {
        try {
            ...
            $.ajax({
                type: 'POST',
                url: '/StorePermissionsLeave/AddItemDetails',
                data: formData,
                contentType: false,
                processData: false,
                success: function (res) {

...
var VoucherReceipt_Module = function () {
    ...
    var initDTVoucherReceipt = function () {
        ...
        table.DataTable({
            ...
            buttons: [
                ...
                {
                    "extend": "excelHtml5",
                    "filename": "سندات قبض بنكية",
                    "title": "سندات قبض بنكية",
                    exportOptions: {
                        columns: ':visible'
                    }
                },
            ...
            ajax: {
                url: '/VoucherBankReceipts/GetAll',
                type: 'GET',
                data(d) {
                    d.accountTreeToId = $("#AccountTreeToId").val();
    ...
    function deleteRow(id) {
        Swal.fire({
            ...
            if (result.value) {
                var url = '/VoucherBankReceipts/Delete';
                $.ajax({
                    type: "POST",
                    url: url,
                    data: {
    ...
    var initDetailsDT = function () {
        ...
        table.DataTable({
            ...
            ajax: {
                url: '/VoucherBankReceipts/GetDSVoucherTransaction',
                type: 'GET',
            ...
            columnDefs: [

                {
                    ...
                    render: function (data, type, row, meta) {
                        return '\
							<div class="btn-group">\
							<a href="javascript:;" onclick=VoucherReceipt_Module.deleteRowComplex('+ row.Id + ')  class="btn btn-sm btn-clean btn-icUrln deleteIcon" title="حذف">\
            ...
            drawCallback: function () {
                var html = ' <tr><th colspan ="6" style= "text-align:center" ><div class="row alert alert-success"><label>اجمالى الدائن : ';
                var api = this.api();
    ...
    function AddVoucherTransaction() {
        try {
            ...
            $.ajax({
                type: 'POST',
                url: '/VoucherBankReceipts/AddVoucherTransaction',
                data: formData,
                contentType: false,
                processData: false,
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.isValid) {
                        ...
                        if (!res.isInsert) {
                            setTimeout(function () { window.location = "/VoucherBankReceipts/Index" }, 3000);
                        } else
                            setTimeout(function () { window.location = "/VoucherBankReceipts/CreateEdit" }, 3000);
                    } else {
                        toastr.error(res.message, '');
                    }
                },

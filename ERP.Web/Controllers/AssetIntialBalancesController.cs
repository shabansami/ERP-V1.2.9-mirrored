        ...
        void AddGeneralDailies(IntialBalanceVM vm, List<GeneralSetting> generalSetting)
        {
            if (vm.DebitCredit == 1) // الرصيد مدين
            {
                ...
                db.GeneralDailies.Add(new GeneralDaily
                {
                    ...
                    Debit = vm.Amount ?? 0,
                    Notes = $"رصيد أول المدة اصول  : {db.AccountsTrees.Where(x => x.Id == vm.AccountId).FirstOrDefault().AccountName} {vm.Notes}",
                    BranchId = vm.BranchId,
                    TransactionDate = vm.DateIntial,
                    TransactionId = vm.AccountId,
                    TransactionTypeId = (int)TransactionsTypesCl.InitialBalanceAsset
                });
                //رأس المال 
                db.GeneralDailies.Add(new GeneralDaily
                {

...
namespace ERP.Web.Controllers
{
    ...
    public class ExpenseTypesController : Controller
    {
        ...
        public ActionResult GetAll()
        {
            ...
            return Json(new
            {
                data = db.ExpenseTypes.Where(x => !x.IsDeleted).OrderBy(x=>x.CreatedOn).Select(x => new { Id = x.Id, CreatedOn=x.CreatedOn,  Name = x.Name, Actions = n, Num = n }).ToList()
        ...
        public ActionResult CreateEdit()
        {
            ...
            {                   // add
                ViewBag.LastRow = db.ExpenseTypes.Where(x => !x.IsDeleted).OrderByDescending(x => x.CreatedOn).FirstOrDefault();
                return View(new ExpenseType());
            }
        }
        [HttpPost]
        public JsonResult CreateEdit(ExpenseType vm)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (vm.Id != Guid.Empty)
                {
                    if (db.ExpenseTypes.Where(x => !x.IsDeleted && x.Name == vm.Name && x.Id != vm.Id).Count() > 0)
                        return Json(new { isValid = false, message = "الاسم موجود مسبقا" });

                    var model = db.ExpenseTypes.FirstOrDefault(x=>x.Id==vm.Id);
                    model.Name = vm.Name;
                ...
                {
                    if (db.ExpenseTypes.Where(x => !x.IsDeleted && x.Name == vm.Name).Count() > 0)
                        return Json(new { isValid = false, message = "الاسم موجود مسبقا" });

                    isInsert = true;
                    //========================
                    long newAccountNum = 0;// new account number 
                    var val = db.GeneralSettings.Find((int)GeneralSettingCl.AccountTreeExpensesAccount).SValue;
                    if (val == null)
                    ...
                    db.ExpenseTypes.Add(vm);
                    //==============================
                    //isSaved = InsertGeneralSettings<ExpenseType>.AddAccountTree(GeneralSettingCl.AccountTreeExpensesAccount, vm.Name, AccountTreeSelectorTypesCl.Expense, vm, auth.CookieValues.UserId);

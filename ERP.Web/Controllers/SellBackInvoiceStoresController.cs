...
namespace ERP.Web.Controllers
{
    ...
    public class SellBackInvoiceStoresController : Controller
    {
        // GET: SellBackInvoiceStores
        ...
        public ActionResult ApprovalStore(string invoGuid)
        {
            ...
            {
                return View(db.SellBackInvoices.Where(x => x.Id == guid).FirstOrDefault());
            }
        ...
        public ActionResult ApprovalInvoice(string invoGuid, string data)
        {
            ...
            if (Guid.TryParse(invoGuid, out Id))
            {
                ...
                if (model != null)
                {
                    ...
                    //اضافة الحالة 
                    var casesSellBackInvoiceHistory = new CasesSellInvoiceHistory
                    {
                    ...
                    foreach (var item in itemDetailsStor)
                    {
                        var purchaseDetails = db.SellBackInvoicesDetails.Where(x => x.Id == item.Id).FirstOrDefault();
                    ...
                    if (itemDetailsStor.Sum(x => x.QuantityReal) != itemDetailsStor.Sum(x => x.Quantity))
                    {
                        ...
                        casesSellBackInvoiceHistory.CaseId = (int)CasesCl.BackInvoiceQuantityDiffReal;
                        db.CasesSellInvoiceHistories.Add(casesSellBackInvoiceHistory);
                    ...
                    else
                    {
                        ...
                        db.Entry(model).State = EntityState.Modified;
                        casesSellBackInvoiceHistory.CaseId = (int)CasesCl.BackInvoiceApprovalStore;
                        db.CasesSellInvoiceHistories.Add(casesSellBackInvoiceHistory);
                        db.SaveChanges(auth.CookieValues.UserId);
